import React, { useState } from "react";

import Qrcode from "../assets/image/Footer/Qrcode.jpg";

import GooglePlayImg from "../assets/image/Footer/google-play.jpg";
import AppStoreImg from "../assets/image/Footer/appstore.jpg";

import { LiaFacebookF } from "react-icons/lia";
import { FiTwitter } from "react-icons/fi";
import { FaInstagram } from "react-icons/fa";
import { RiLinkedinLine } from "react-icons/ri";
import { PiCopyright } from "react-icons/pi";
import { VscSend } from "react-icons/vsc";

export default function Footer() {

  
  return (
<div className="bg-black text-white py-8">
  <div className="container mx-auto flex flex-wrap gap-8 justify-between items-start text-start">
    {/* Exclusive Section */}
    <div className="flex flex-col gap-4 w-full sm:w-auto text-center sm:text-start">
      <h2 className="font-bold text-2xl">Exclusive</h2>
      <p className="text-[20px] font-medium pt-1.5">Subscribe</p>
      <p className="footer_text">Get 10% off your first order</p>
      <div className="flex w-full sm:w-[215px] h-12 items-center rounded border-2 border-white px-4 mx-auto sm:mx-0">
        <input
          type="email"
          className="w-full bg-transparent outline-none border-none placeholder-zinc-600"
          placeholder="Enter your email"
        />
        <VscSend />
      </div>
    </div>

    {/* Support Section */}
    <div className="flex flex-col gap-4 w-full sm:w-auto text-center sm:text-start">
      <h3 className="text-[20px] font-medium">Support</h3>
      <p className="pt-1.5 footer_text">
        111 Bijoy Sarani, Dhaka,
        <br /> DH 1515, Bangladesh.
      </p>
      <p className="footer_text">exclusive@gmail.com</p>
      <p className="footer_text">+88015-88888-9999</p>
    </div>

    {/* Account Section */}
    <div className="flex flex-col gap-4 w-full sm:w-auto text-center sm:text-start">
      <h3 className="text-[20px] font-medium">Account</h3>
      <p className="footer_text">My Account</p>
      <p className="footer_text">Login / Register</p>
      <p className="footer_text">Cart</p>
      <p className="footer_text">Wishlist</p>
      <p className="footer_text">Shop</p>
    </div>

    {/* Quick Links Section */}
    <div className="flex flex-col gap-4 w-full sm:w-auto text-center sm:text-start">
      <h3 className="text-[20px] font-medium">Quick Link</h3>
      <p className="footer_text">Privacy Policy</p>
      <p className="footer_text">Terms Of Use</p>
      <p className="footer_text">FAQ</p>
      <p className="footer_text">Contact</p>
    </div>

    {/* Download App Section */}
    <div className="flex flex-col gap-4 w-full sm:w-auto text-center sm:text-start">
      <h3 className="text-[20px] font-medium">Download App</h3>
      <p className="pt-1.5 text-xs font-medium text-zinc-400">
        Save $3 with App New User Only{" "}
      </p>
      <div className="flex gap-2 items-center justify-center sm:justify-start">
        <div>
          <img src={Qrcode} alt="QR Code" />
        </div>
        <div className="flex flex-col gap-1">
          <img src={GooglePlayImg} alt="Google Play Store" />
          <img src={AppStoreImg} alt="App Store" />
        </div>
      </div>
      <div className="flex gap-6 text-xl pt-2 justify-center sm:justify-start">
        <LiaFacebookF />
        <FiTwitter />
        <FaInstagram />
        <RiLinkedinLine />
      </div>
    </div>
  </div>

  {/* Copyright Section */}
  <div className="flex justify-center items-center text-center gap-[6px] mt-8 text-zinc-500 pb-4">
    <PiCopyright className="text-[22px]" />
    <p>Copyright Rimel 2022. All rights reserved.</p>
  </div>
</div>


  );
}
